            ...
            {messages.length === 0 ? (
              ...
              messages.map(msg => (
                <tr key={msg.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-slate-600 max-w-xs" title={msg.message}>
                    <div className="whitespace-pre-wrap text-xs leading-relaxed">
                      {msg.message.split('\n').map((line, i) => {
                        const linkRegex = /(https?:\/\/[^\s]+|\/artist\/[^\s]+)/g;
                        const parts = line.split(linkRegex);
                        return (
                          <React.Fragment key={i}>
                            {parts.map((part, j) => 
                              linkRegex.test(part) ? (
                                <a key={j} href={part} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline" dir="ltr" style={{display: 'inline-block'}}>{part}</a>
                              ) : (
