...
export default function ReviewArtistEditsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleAction = async (actionType: 'approveEdits' | 'rejectEdits', feedback: string = '') => {
    if (actionType === 'rejectEdits' && !feedback) {
      setModalOpen(true);
      return;
    }
  ...
  const handleConfirmReject = (reason: string) => {
    setModalOpen(false);
    handleAction('rejectEdits', reason);
  };
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto" dir="rtl">
      <AdminPromptModal 
        isOpen={modalOpen}
        title="دلیل رد کردن تغییرات"
        placeholder="لطفا دلیل رد کردن را بنویسید تا هنرمند بتواند آن را اصلاح کند..."
